...
export default {
	data: new SlashCommandBuilder()
		...
		.addIntegerOption((option) =>
			option.setName("volume").setDescription(i18n.__("volume.description"))
		),
	execute(interaction: ChatInputCommandInteraction) {
		...
		if (!queue)
			return interaction
				.reply({ content: i18n.__("volume.errorNotQueue"), ephemeral: true })
				.catch(console.error);

		if (!canModifyQueue(guildMemer!))
			return interaction
				.reply({ content: i18n.__("volume.errorNotChannel"), ephemeral: true })
				.catch(console.error);
		...
		if (isNaN(volumeArg))
			return interaction
				.reply({ content: i18n.__("volume.errorNotNumber"), ephemeral: true })
				.catch(console.error);

		if (Number(volumeArg) > 100 || Number(volumeArg) < 0)
			return interaction
				.reply({ content: i18n.__("volume.errorNotValid"), ephemeral: true })
				.catch(console.error);
